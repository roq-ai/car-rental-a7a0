...
import { getUserProjects } from 'apiSdk/user-projects';
...
function DashboardPage() {
  ...
  const userProjectStat = useSWR<PaginatedInterface<UserProjectInterface>>(
    () => `/user-projects`,
    () => getUserProjects({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'User Projects',
      result: userProjectStat?.data?.totalCount,
      isLoading: userProjectStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Tasks',
