...
type ColumnType = ColumnDef<UserProjectInterface, unknown>;

interface UserProjectListPageProps {
...
export function UserProjectListPage(props: UserProjectListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<UserProjectInterface>>(
    () => `/user-projects?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: UserProjectInterface) => {
    if (hasAccess('user_project', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/user-projects/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/user-projects/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('user_project', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/user-projects/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="user_project" />
        </Flex>

        {hasAccess('user_project', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/user-projects/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
